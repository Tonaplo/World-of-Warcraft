
LUIDB = {
	["namespaces"] = {
		["Swing"] = {
			["profiles"] = {
				["Base"] = {
					["Enable"] = false,
					["Text"] = {
						["Font"] = "Prototype",
						["Enable"] = true,
					},
					["General"] = {
						["X"] = -90,
					},
				},
			},
		},
		["RaidMenu"] = {
			["profiles"] = {
				["Base"] = {
					["ShowToolTips"] = true,
				},
			},
		},
		["Vengeance"] = {
		},
		["Art Work"] = {
		},
		["Bags"] = {
			["profiles"] = {
				["Base"] = {
					["Enable"] = false,
					["Bags"] = {
						["CoordX"] = 1560.37158203125,
						["CoordY"] = 784.483276367188,
					},
					["Reagents"] = {
						["CoordX"] = 1560.37158203125,
						["ShowNew"] = false,
						["ItemQuality"] = false,
						["CoordY"] = 784.483276367188,
						["FontSize"] = 12,
						["Font"] = "AvantGarde_LT_Medium",
						["ShowQuest"] = true,
					},
					["Bank"] = {
						["CoordX"] = 1560.37158203125,
						["ShowNew"] = false,
						["ItemQuality"] = false,
						["CoordY"] = 784.483276367188,
						["FontSize"] = 12,
						["Font"] = "AvantGarde_LT_Medium",
						["ShowQuest"] = true,
					},
				},
			},
		},
		["Infotext"] = {
			["realm"] = {
				["Icecrown"] = {
					["Gold"] = {
						["Alliance"] = {
							["Farahn"] = 207795770,
							["Greybone"] = 987721158,
							["Faerand"] = 332757806,
							["Korugen"] = 8556477,
							["Kaldorath"] = 1274216901,
							["Mallusof"] = 1110190831,
							["Thedodrouk"] = 7422272,
							["Rahran"] = 595705761,
							["Praerend"] = 1499245138,
						},
					},
				},
				["Frostwolf"] = {
					["Gold"] = {
						["Alliance"] = {
							["Fandagny"] = 366811,
							["Caircaidin"] = 2000,
							["Tyindel"] = 10361,
						},
					},
				},
				["Bleeding Hollow"] = {
					["Gold"] = {
						["Horde"] = {
							["Zaliga"] = 8607092,
							["Evisca"] = 303004,
							["Rentror"] = 470,
							["Gorog"] = 993959,
						},
					},
				},
				["Hyjal"] = {
					["Gold"] = {
						["Horde"] = {
							["Deminath"] = 2131,
						},
					},
				},
				["Proudmoore"] = {
					["Gold"] = {
						["Horde"] = {
							["Deminath"] = 1644198054,
							["Morifa"] = 1384217305,
							["Kindhoof"] = 1186116499,
						},
						["Alliance"] = {
							["Miriyal"] = 373580598,
							["Farahn"] = 207174970,
							["Greybone"] = 987100358,
							["Amranar"] = 599870423,
							["Granjior"] = 1186116499,
							["Kaldorath"] = 1269646101,
							["Zearthan"] = 38822533,
							["Korugen"] = 8422477,
							["Mallusof"] = 1162234125,
							["Fimwack"] = 1625871926,
							["Morifa"] = 1385014335,
							["Rahran"] = 594441111,
							["Faerand"] = 331462352,
							["Praerend"] = 1500267344,
						},
					},
				},
				["Tichondrius"] = {
					["Gold"] = {
						["Horde"] = {
							["Alura"] = 3644797,
							["Lorvol"] = 1057540,
							["Morifa"] = 1396339366,
							["Zaliga"] = 8607092,
							["Kindhoof"] = 1198250606,
						},
						["Alliance"] = {
							["Mallusof"] = 226844308,
							["Korugen"] = 7835929,
							["Zethan"] = 38755956,
						},
					},
				},
				["Aggramar"] = {
					["Gold"] = {
						["Horde"] = {
							["Gottagetlia"] = 319950,
						},
					},
				},
			},
			["profiles"] = {
				["Base"] = {
					["Durability"] = {
						["Font"] = "Prototype",
					},
					["Guild"] = {
						["sortASC"] = {
							false, -- [1]
						},
						["Font"] = "Prototype",
						["sortCols"] = {
							"note", -- [1]
							"class", -- [2]
						},
					},
					["Bags"] = {
						["Font"] = "Prototype",
					},
					["Currency"] = {
						["Enable"] = true,
						["X"] = 250,
						["Display"] = 1586,
						["Font"] = "Prototype",
					},
					["Gold"] = {
						["Font"] = "Prototype",
					},
					["Clock"] = {
						["Time24"] = true,
						["Font"] = "Prototype",
					},
					["Instance"] = {
						["Enable"] = true,
						["Font"] = "Prototype",
					},
					["FPS"] = {
						["Font"] = "Prototype",
					},
					["Friends"] = {
						["Font"] = "Prototype",
					},
					["Memory"] = {
						["Enable"] = false,
						["Font"] = "Prototype",
					},
					["DPS"] = {
						["Enable"] = false,
						["Font"] = "Prototype",
					},
					["CombatLock"] = true,
					["DualSpec"] = {
						["Enable"] = true,
						["ShowSpentPoints"] = false,
						["X"] = -600,
						["InfoPanel"] = {
							["Vertical"] = "Top",
							["Horizontal"] = "Right",
						},
						["Font"] = "Prototype",
					},
					["MoveSpeed"] = {
						["X"] = 690,
						["InfoPanel"] = {
							["Horizontal"] = "Left",
						},
						["Font"] = "Prototype",
					},
				},
			},
		},
		["Unitframes"] = {
			["global"] = {
				["Mallusof"] = {
					["Player"] = {
						["Y"] = -200.000091552734,
						["X"] = -310.519439697266,
						["Castbar"] = {
							["General"] = {
								["Y"] = -148.80192565918,
								["X"] = -365.631866455078,
								["Point"] = "CENTER",
							},
						},
					},
					["Party"] = {
						["Y"] = -55.5457916259766,
						["X"] = 159.210083007813,
					},
					["Settings"] = {
						["ShowV2PartyTextures"] = false,
						["ShowV2ArenaTextures"] = false,
					},
					["Focus"] = {
						["Y"] = 248.361129760742,
						["X"] = 239.659820556641,
						["Point"] = "LEFT",
					},
					["Target"] = {
						["Y"] = -205.116638183594,
						["X"] = 381.128936767578,
						["Castbar"] = {
							["General"] = {
								["Y"] = -159.178176879883,
								["X"] = -367.784484863281,
								["Point"] = "RIGHT",
							},
						},
					},
					["ToT"] = {
						["Y"] = -249.999954223633,
						["X"] = -415.672241210938,
						["Point"] = "RIGHT",
					},
					["Version"] = 3600,
					["Boss"] = {
						["Y"] = 205.941268920898,
						["X"] = 453.880889892578,
						["Point"] = "BOTTOMLEFT",
					},
				},
			},
			["profiles"] = {
				["Base"] = {
					["ToToT"] = {
						["Y"] = -83.404296875,
						["X"] = 370.853759765625,
					},
					["Party"] = {
						["Enable"] = false,
						["ShowPlayer"] = true,
						["X"] = 130.55744934082,
						["Castbar"] = {
							["General"] = {
								["Enable"] = false,
							},
						},
						["Y"] = 35.9801292419434,
						["Fader"] = {
							["UseGlobalSettings"] = false,
						},
						["Bars"] = {
							["Health"] = {
								["Color"] = "Gradient",
							},
						},
					},
					["Settings"] = {
						["ShowV2Textures"] = false,
						["show_v2_textures"] = false,
						["ShowV2BossTextures"] = false,
						["ShowV2PartyTextures"] = false,
						["show_v2_arena_textures"] = false,
						["show_v2_party_textures"] = false,
						["ShowV2ArenaTextures"] = false,
					},
					["Focus"] = {
						["Point"] = "LEFT",
						["Castbar"] = {
							["General"] = {
								["Width"] = 400,
								["Y"] = 350,
								["X"] = 640,
								["Height"] = 35,
								["IndividualColor"] = true,
							},
							["Shield"] = {
								["Color"] = {
									["a"] = 0.680000007152557,
									["b"] = 0,
									["g"] = 1,
									["r"] = 0,
								},
								["BarColor"] = {
									["a"] = 0.680000007152557,
									["b"] = 0,
									["g"] = 0,
									["r"] = 1,
								},
								["IndividualBorder"] = false,
								["IndividualColor"] = true,
								["Texture"] = "Blizzard Dialog",
							},
							["Text"] = {
								["Name"] = {
									["Font"] = "Prototype",
									["OffsetX"] = 0,
									["Size"] = 15,
								},
								["Time"] = {
									["Font"] = "Prototype",
									["ShowMax"] = true,
									["Size"] = 15,
								},
							},
							["Colors"] = {
								["Bar"] = {
									["a"] = 0.680000007152557,
									["b"] = 0,
									["g"] = 1,
									["r"] = 0,
								},
							},
						},
						["Y"] = -50,
						["X"] = 239.659820556641,
						["Aura"] = {
							["Debuffs"] = {
								["GrowthX"] = "LEFT",
								["Y"] = 60,
								["InitialAnchor"] = "TOPRIGHT",
								["GrowthY"] = "UP",
								["Size"] = 60,
							},
							["Buffs"] = {
								["Enable"] = true,
							},
						},
						["Height"] = 100,
						["Bars"] = {
							["Health"] = {
								["Color"] = "Individual",
								["Height"] = 100,
								["IndividualColor"] = {
									["r"] = 0,
									["g"] = 1,
									["b"] = 0,
								},
							},
						},
						["Texts"] = {
							["Health"] = {
								["Enable"] = true,
								["Y"] = 0,
								["RelativePoint"] = "BOTTOMLEFT",
								["IndividualColor"] = {
									["b"] = 0.890196078431373,
									["g"] = 1,
									["r"] = 0.941176470588235,
								},
								["Size"] = 12,
							},
							["Power"] = {
								["Point"] = "LEFT",
								["Enable"] = true,
								["Y"] = 0,
								["Size"] = 14,
							},
						},
					},
					["Target"] = {
						["Portrait"] = {
							["Y"] = 10,
							["X"] = 300,
							["Height"] = 100,
							["Width"] = 100,
						},
						["Castbar"] = {
							["General"] = {
								["Point"] = "CENTER",
								["Width"] = 380,
								["Y"] = -180,
								["X"] = 20,
								["Height"] = 25,
								["IndividualColor"] = true,
							},
							["Shield"] = {
								["Color"] = {
									["a"] = 1,
									["b"] = 0,
									["g"] = 0,
									["r"] = 1,
								},
								["BarColor"] = {
									["a"] = 0.680000007152557,
									["r"] = 0.580392156862745,
									["g"] = 0.580392156862745,
									["b"] = 0.580392156862745,
								},
								["IndividualColor"] = true,
							},
							["Text"] = {
								["Name"] = {
									["Font"] = "Prototype",
								},
								["Time"] = {
									["Font"] = "Prototype",
									["ShowMax"] = false,
									["Size"] = 15,
								},
							},
							["Colors"] = {
								["Shield"] = {
									["a"] = 0.1,
									["r"] = 0.5,
									["g"] = 0,
									["b"] = 0,
								},
								["Bar"] = {
									["a"] = 0.680000007152557,
									["r"] = 1,
									["g"] = 0,
									["b"] = 0,
								},
							},
						},
						["Icons"] = {
							["PvP"] = {
								["Enable"] = true,
							},
						},
						["X"] = 350.000030517578,
						["Aura"] = {
							["Buffs"] = {
								["Num"] = 8,
								["GrowthX"] = "LEFT",
								["DisableCooldown"] = true,
								["ColorByType"] = true,
								["Y"] = -35,
								["X"] = 0,
								["InitialAnchor"] = "BOTTOMRIGHT",
								["GrowthY"] = "DOWN",
								["AuraTimer"] = true,
							},
							["Debuffs"] = {
								["GrowthX"] = "RIGHT",
								["PlayerOnly"] = true,
								["Y"] = -35,
								["X"] = 0,
								["InitialAnchor"] = "BOTTOMLEFT",
								["GrowthY"] = "DOWN",
								["IncludePet"] = true,
								["FadeOthers"] = false,
							},
						},
						["Bars"] = {
							["TotalAbsorb"] = {
								["Enable"] = true,
							},
							["Health"] = {
								["IndividualColor"] = {
									["r"] = 0,
									["g"] = 1,
									["b"] = 0,
								},
							},
							["Power"] = {
								["Color"] = "By Type",
							},
							["HealPrediction"] = {
								["Enable"] = true,
							},
							["ComboPoints"] = {
								["BackgroundColor"] = {
									["Enable"] = false,
								},
							},
						},
						["Border"] = {
							["EdgeSize"] = 8,
							["EdgeFile"] = "1 Pixel",
							["Aggro"] = true,
						},
						["Texts"] = {
							["PowerPercent"] = {
								["Enable"] = true,
								["Y"] = -15,
								["IndividualColor"] = {
									["r"] = 1,
									["g"] = 1,
									["b"] = 1,
								},
								["Size"] = 14,
							},
							["Name"] = {
								["ColorNameByClass"] = false,
								["Point"] = "TOPRIGHT",
								["X"] = -5,
								["RelativePoint"] = "TOPRIGHT",
								["Format"] = "Name + Level",
								["ShortClassification"] = true,
								["Size"] = 14,
							},
							["Power"] = {
								["Enable"] = false,
								["IndividualColor"] = {
									["r"] = 1,
									["g"] = 1,
									["b"] = 1,
								},
							},
							["Health"] = {
								["Point"] = "LEFT",
								["Y"] = 0,
								["RelativePoint"] = "LEFT",
								["Format"] = "Standard Short",
								["Size"] = 12,
							},
							["HealthPercent"] = {
								["Point"] = "RIGHT",
								["ShowAlways"] = true,
								["Y"] = 0,
								["RelativePoint"] = "RIGHT",
								["Size"] = 12,
							},
							["HealthMissing"] = {
								["IndividualColor"] = {
									["r"] = 1,
									["g"] = 1,
									["b"] = 1,
								},
								["Size"] = 15,
							},
							["PowerMissing"] = {
								["IndividualColor"] = {
									["r"] = 1,
									["g"] = 1,
									["b"] = 1,
								},
								["Size"] = 13,
							},
						},
					},
					["XP_Rep"] = {
						["Reputation"] = {
							["AlwaysShow"] = true,
							["Enable"] = false,
						},
						["General"] = {
							["Font"] = "Prototype",
						},
					},
					["PetTarget"] = {
						["Point"] = "LEFT",
						["Y"] = -243.912719726563,
						["X"] = 35.2860870361328,
					},
					["PartyTarget"] = {
						["Enable"] = false,
					},
					["Layout"] = "Mallusof",
					["Pet"] = {
						["Point"] = "BOTTOMRIGHT",
						["Y"] = 310,
						["X"] = -50,
					},
					["Boss"] = {
						["Castbar"] = {
							["X"] = -310,
							["General"] = {
								["Enable"] = true,
							},
							["Text"] = {
								["Name"] = {
									["Font"] = "Prototype",
								},
								["Time"] = {
									["Font"] = "Prototype",
								},
							},
						},
						["Point"] = "RIGHT",
						["Y"] = 300,
						["X"] = -400,
						["Padding"] = 40,
						["Bars"] = {
							["Health"] = {
								["IndividualColor"] = {
									["r"] = 0,
									["g"] = 1,
									["b"] = 0,
								},
							},
							["Power"] = {
								["Enable"] = true,
							},
						},
						["Texts"] = {
							["Name"] = {
								["Point"] = "LEFT",
								["RelativePoint"] = "LEFT",
							},
							["HealthPercent"] = {
								["Point"] = "RIGHT",
								["Enable"] = true,
								["ShowAlways"] = true,
								["RelativePoint"] = "RIGHT",
								["IndividualColor"] = {
									["r"] = 1,
									["g"] = 1,
									["b"] = 1,
								},
								["Size"] = 17,
							},
						},
					},
					["BossTarget"] = {
						["Castbar"] = {
							["General"] = {
								["Enable"] = false,
							},
						},
					},
					["Player"] = {
						["Castbar"] = {
							["General"] = {
								["Point"] = "CENTER",
								["Width"] = 380,
								["Y"] = -210,
								["X"] = 20,
								["Height"] = 25,
								["IndividualColor"] = true,
							},
							["Text"] = {
								["Name"] = {
									["Font"] = "Prototype",
								},
								["Time"] = {
									["Font"] = "Prototype",
									["Size"] = 15,
								},
							},
							["Colors"] = {
								["Bar"] = {
									["a"] = 1,
									["r"] = 0,
									["g"] = 1,
									["b"] = 0,
								},
							},
						},
						["Icons"] = {
							["Combat"] = {
								["Y"] = -20,
								["Enable"] = true,
							},
							["Resting"] = {
								["Enable"] = true,
							},
						},
						["Y"] = -200.000091552734,
						["X"] = -350.000030517578,
						["Height"] = 54,
						["Bars"] = {
							["WarlockBar"] = {
								["Y"] = 100,
								["X"] = 350,
								["Lock"] = false,
								["Padding"] = 1,
							},
							["Power"] = {
								["Color"] = "By Type",
								["Y"] = -30,
								["Height"] = 20,
							},
							["HealPrediction"] = {
								["Enable"] = true,
							},
							["DruidMana"] = {
								["Height"] = 20,
							},
							["Chi"] = {
								["Y"] = 90,
								["X"] = 375,
								["Lock"] = false,
								["Texture"] = "oUF LUI",
								["Width"] = 200,
							},
							["TotalAbsorb"] = {
								["Enable"] = true,
							},
							["Health"] = {
								["IndividualColor"] = {
									["r"] = 0,
									["g"] = 1,
									["b"] = 0,
								},
							},
							["SoulShards"] = {
								["Y"] = -4,
								["X"] = 25,
								["Lock"] = false,
								["Padding"] = 1,
								["Texture"] = "Otravi",
								["Width"] = 200,
							},
							["ClassIcons"] = {
							},
							["AltPower"] = {
								["Enable"] = true,
								["Height"] = 15,
							},
						},
						["Border"] = {
							["EdgeSize"] = 8,
							["EdgeFile"] = "1 Pixel",
							["Aggro"] = true,
						},
						["Texts"] = {
							["Power"] = {
								["Enable"] = false,
							},
							["DruidMana"] = {
								["Enable"] = false,
							},
							["AltPower"] = {
								["Enable"] = true,
								["Font"] = "Prototype",
							},
							["PowerPercent"] = {
								["Point"] = "TOP",
								["Y"] = -2,
								["Enable"] = true,
							},
							["Health"] = {
								["Enable"] = false,
							},
						},
					},
					["Version"] = 3600,
					["Raid"] = {
						["Point"] = "BOTTOM",
						["Enable"] = false,
						["Backdrop"] = {
							["Color"] = {
								["a"] = 0.110000014305115,
							},
							["Texture"] = "None",
						},
						["Width"] = 80,
						["Y"] = 150,
						["X"] = 0,
						["Padding"] = 1,
						["Icons"] = {
							["Raid"] = {
								["Y"] = 0,
								["Enable"] = true,
								["Size"] = 60,
							},
						},
						["GroupPadding"] = 1,
						["Bars"] = {
							["TotalAbsorb"] = {
								["Enable"] = true,
							},
							["Health"] = {
								["Color"] = "Gradient",
								["Width"] = 80,
								["Height"] = 35,
							},
							["Power"] = {
								["Color"] = "By Type",
								["Width"] = 80,
								["Y"] = -35,
							},
							["HealPrediction"] = {
								["Enable"] = true,
							},
						},
						["Height"] = 40,
						["Texts"] = {
							["HealthPercent"] = {
								["ShowAlways"] = true,
								["Y"] = -10,
								["Color"] = "Gradient",
								["X"] = 0,
							},
						},
					},
					["Colors"] = {
						["Smooth"] = {
							{
								0.690196078431373, -- [1]
								0.309803921568627, -- [2]
								0.309803921568627, -- [3]
								1, -- [4]
							}, -- [1]
							{
								nil, -- [1]
								nil, -- [2]
								nil, -- [3]
								1, -- [4]
							}, -- [2]
						},
					},
					["Maintank"] = {
						["Y"] = 350.000030517578,
					},
					["PartyPet"] = {
						["Enable"] = false,
					},
					["ToT"] = {
						["Y"] = -115.207145690918,
						["X"] = 371.490386962891,
					},
				},
				["Korugen - Icecrown"] = {
					["Player"] = {
						["Bars"] = {
							["ClassIcons"] = {
							},
						},
					},
				},
			},
		},
		["Threat"] = {
			["profiles"] = {
				["Base"] = {
					["Text"] = {
						["Precision"] = 2,
					},
					["General"] = {
						["artifact"] = true,
						["Width"] = 410,
						["TankHide"] = false,
						["X"] = 770,
						["expMode"] = true,
						["showRested"] = true,
						["Y"] = 5,
						["ShowAbsolute"] = true,
					},
					["Appearance"] = {
						["BGTexture"] = "PlaterBackground",
						["Color"] = "Individual",
						["IndividualColor"] = {
							["g"] = 0.847058823529412,
							["b"] = 0,
						},
						["BGMultiplier"] = 0,
					},
				},
			},
		},
		["Mirror Bar"] = {
			["profiles"] = {
				["Base"] = {
					["Text"] = {
						["Name"] = {
							["Font"] = "Prototype",
						},
						["Time"] = {
							["Font"] = "Prototype",
						},
					},
					["General"] = {
						["ArchyBar"] = true,
					},
				},
			},
		},
		["RemoveThatBuff"] = {
			["profiles"] = {
				["Base"] = {
					["Enable"] = true,
				},
			},
		},
		["Merchant"] = {
			["profiles"] = {
				["Base"] = {
					["AutoSell"] = {
						["Enable"] = true,
					},
				},
			},
		},
		["Auras"] = {
			["profiles"] = {
				["Base"] = {
					["Buffs"] = {
						["Consolidate"] = false,
						["Duration"] = {
							["Font"] = "Prototype",
						},
						["Y"] = -75,
						["X"] = -200,
						["Count"] = {
							["Font"] = "Prototype",
						},
						["Anchor"] = "TOPRIGHT",
					},
					["Debuffs"] = {
						["HorizontalSpacing"] = 3,
						["Duration"] = {
							["Font"] = "Prototype",
							["Size"] = 18,
						},
						["Y"] = -785,
						["X"] = -1400,
						["Count"] = {
							["Font"] = "Prototype",
						},
						["SortMethod"] = "Index",
						["Anchor"] = "TOPRIGHT",
						["AurasPerRow"] = 40,
						["Size"] = 60,
					},
				},
			},
		},
		["Panels"] = {
			["profiles"] = {
				["Korugen - Icecrown"] = {
					["Dps"] = {
						["Anchor"] = "DetailsBaseFrame1",
						["OffsetY"] = 0,
						["Additional"] = "DetailsRowFrame1",
					},
					["Minimap"] = {
						["IsShown"] = true,
					},
					["MicroMenu"] = {
						["IsShown"] = true,
					},
				},
				["Farahn - Icecrown"] = {
					["Dps"] = {
						["Anchor"] = "DetailsBaseFrame1",
						["OffsetY"] = 0,
						["Additional"] = "DetailsRowFrame1",
					},
					["Minimap"] = {
						["IsShown"] = true,
					},
					["MicroMenu"] = {
						["IsShown"] = true,
					},
				},
				["Greybone - Icecrown"] = {
					["Dps"] = {
						["Anchor"] = "DetailsBaseFrame1",
						["OffsetY"] = 0,
						["Additional"] = "DetailsRowFrame1",
					},
					["MicroMenu"] = {
						["IsShown"] = true,
					},
					["Minimap"] = {
						["IsShown"] = true,
					},
				},
				["Base"] = {
					["Tps"] = {
						["Width"] = 600,
					},
					["MicroMenu"] = {
						["IsShown"] = true,
					},
					["Raid"] = {
						["Direction"] = "SOLID",
						["Animation"] = "None",
						["IsShown"] = true,
						["Width"] = 370,
						["Anchor"] = "CompactRaidFrameContainer",
						["Height"] = 205,
					},
					["Chat"] = {
						["AlwaysShow"] = true,
						["IsShown"] = true,
					},
					["Minimap"] = {
						["IsShown"] = true,
					},
				},
				["Kaldorath - Icecrown"] = {
					["Dps"] = {
						["Anchor"] = "DetailsBaseFrame1",
						["OffsetY"] = 0,
						["Additional"] = "DetailsRowFrame1",
					},
					["Minimap"] = {
						["IsShown"] = true,
					},
					["MicroMenu"] = {
						["IsShown"] = true,
					},
				},
			},
		},
		["Forte"] = {
		},
		["Themes"] = {
			["profiles"] = {
				["Korugen - Icecrown"] = {
					["micromenu_btn"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.8, -- [4]
					},
					["color_bottom"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.5, -- [4]
					},
					["tpsborder"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["tps"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["chat"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["bar"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.7, -- [4]
					},
					["micromenu_bg"] = {
						0.66, -- [1]
						0, -- [2]
						0.43, -- [3]
						0.8, -- [4]
					},
					["orb_cycle"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["color_top"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.5, -- [4]
					},
					["raid"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["theme"] = "Paladin",
					["micromenu_bg2"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						0.7, -- [4]
					},
					["chatborder"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["minimap"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						1, -- [4]
					},
					["editbox"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["bar2"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.6, -- [4]
					},
					["dpsborder"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["dps"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["micromenu_btn_hover"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.8, -- [4]
					},
					["orb_hover"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["chat2"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["sidebar"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["raidborder"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["micromenu"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
					},
					["navi"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.6, -- [4]
					},
					["chat2border"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
					["orb"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
					},
					["navi_hover"] = {
						0.96, -- [1]
						0.21, -- [2]
						0.73, -- [3]
						0.4, -- [4]
					},
				},
				["Farahn - Icecrown"] = {
					["micromenu_btn"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.8, -- [4]
					},
					["color_bottom"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.5, -- [4]
					},
					["tpsborder"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["tps"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["chat"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["bar"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.7, -- [4]
					},
					["micromenu_bg"] = {
						0, -- [1]
						0.09, -- [2]
						0.68, -- [3]
						0.8, -- [4]
					},
					["orb_cycle"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["color_top"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.5, -- [4]
					},
					["raid"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["theme"] = "Shaman",
					["micromenu_bg2"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						0.8, -- [4]
					},
					["chatborder"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["minimap"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						1, -- [4]
					},
					["editbox"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["bar2"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.6, -- [4]
					},
					["dpsborder"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["dps"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["raidborder"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["chat2"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["orb_hover"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["sidebar"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["micromenu_btn_hover"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.8, -- [4]
					},
					["micromenu"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
					},
					["navi"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.6, -- [4]
					},
					["chat2border"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
					["orb"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
					},
					["navi_hover"] = {
						0.04, -- [1]
						0.39, -- [2]
						0.98, -- [3]
						0.4, -- [4]
					},
				},
				["Greybone - Icecrown"] = {
					["micromenu_btn"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.8, -- [4]
					},
					["color_bottom"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.5, -- [4]
					},
					["tpsborder"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["tps"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["chat"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["navi_hover"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["micromenu_bg"] = {
						0.7, -- [1]
						0, -- [2]
						0, -- [3]
						0.8, -- [4]
					},
					["orb_cycle"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["color_top"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.5, -- [4]
					},
					["raid"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["theme"] = "Death Knight",
					["micromenu_bg2"] = {
						0.1, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.8, -- [4]
					},
					["orb"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
					},
					["chat2border"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["navi"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.6, -- [4]
					},
					["bar2"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.6, -- [4]
					},
					["dpsborder"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["dps"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["chat2"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["micromenu_btn_hover"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.8, -- [4]
					},
					["raidborder"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["sidebar"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["orb_hover"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["micromenu"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
					},
					["editbox"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["minimap"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						1, -- [4]
					},
					["chatborder"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.4, -- [4]
					},
					["bar"] = {
						0.8, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.8, -- [4]
					},
				},
				["Base"] = {
					["micromenu_btn"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.8, -- [4]
					},
					["color_bottom"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.5, -- [4]
					},
					["tpsborder"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.4, -- [4]
					},
					["tps"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.4, -- [4]
					},
					["chat"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.4, -- [4]
					},
					["navi_hover"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.4, -- [4]
					},
					["micromenu_bg"] = {
						0.6, -- [1]
						0.6, -- [2]
						0.6, -- [3]
						0.8, -- [4]
					},
					["orb_cycle"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.4, -- [4]
					},
					["color_top"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.5, -- [4]
					},
					["raid"] = {
						0.901960784313726, -- [1]
						0.901960784313726, -- [2]
						0.901960784313726, -- [3]
						0, -- [4]
					},
					["theme"] = "Priest",
					["micromenu_bg2"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						0.7, -- [4]
					},
					["orb"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
					},
					["chat2border"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.4, -- [4]
					},
					["navi"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.6, -- [4]
					},
					["bar2"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						0.600000023841858, -- [4]
					},
					["dpsborder"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						0.400000035762787, -- [4]
					},
					["dps"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.4, -- [4]
					},
					["chat2"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.4, -- [4]
					},
					["micromenu_btn_hover"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.8, -- [4]
					},
					["raidborder"] = {
						0.901960784313726, -- [1]
						0.901960784313726, -- [2]
						0.901960784313726, -- [3]
						0.300000011920929, -- [4]
					},
					["sidebar"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.4, -- [4]
					},
					["orb_hover"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.4, -- [4]
					},
					["micromenu"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
					},
					["editbox"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.4, -- [4]
					},
					["minimap"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						1, -- [4]
					},
					["chatborder"] = {
						0.9, -- [1]
						0.9, -- [2]
						0.9, -- [3]
						0.4, -- [4]
					},
					["bar"] = {
						0, -- [1]
						0, -- [2]
						0, -- [3]
						1, -- [4]
					},
				},
				["Kaldorath - Icecrown"] = {
					["micromenu_btn"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.8, -- [4]
					},
					["color_bottom"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.5, -- [4]
					},
					["tpsborder"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["tps"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["chat"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["bar"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.8, -- [4]
					},
					["micromenu_bg"] = {
						0.48, -- [1]
						0.13, -- [2]
						0.62, -- [3]
						0.8, -- [4]
					},
					["orb_cycle"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["color_top"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.5, -- [4]
					},
					["raid"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["theme"] = "Demon Hunter",
					["micromenu_bg2"] = {
						0.1, -- [1]
						0.1, -- [2]
						0.1, -- [3]
						0.8, -- [4]
					},
					["chatborder"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["minimap"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						1, -- [4]
					},
					["editbox"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["bar2"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.6, -- [4]
					},
					["dpsborder"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["dps"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["micromenu_btn_hover"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.8, -- [4]
					},
					["orb_hover"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["chat2"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["sidebar"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["raidborder"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["micromenu"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
					},
					["navi"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.6, -- [4]
					},
					["chat2border"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
					["orb"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
					},
					["navi_hover"] = {
						0.64, -- [1]
						0.19, -- [2]
						0.79, -- [3]
						0.4, -- [4]
					},
				},
			},
		},
		["Fader"] = {
			["profiles"] = {
				["Base"] = {
					["ForceGlobalSettings"] = false,
					["GlobalSettings"] = {
						["Combat"] = false,
						["Power"] = false,
						["Casting"] = false,
						["Health"] = false,
						["Targeting"] = false,
					},
				},
			},
		},
		["Micromenu"] = {
		},
		["Chat"] = {
			["namespaces"] = {
				["StickyChannels"] = {
					["profiles"] = {
						["Base"] = {
							["Channels"] = {
								["RAID_WARNING"] = true,
							},
						},
					},
				},
				["EditBox"] = {
					["factionrealm"] = {
						["Alliance - Proudmoore"] = {
							"/1 Original meaning of gay is happy, so yeah, I guess you're right :)", -- [1]
							"/gdisband", -- [2]
							"/1 Original meaning of gay is happy, so yeah, I guess you're right :)", -- [3]
							"/gdisband", -- [4]
							"/rchars", -- [5]
							"/1 Original meaning of gay is happy, so yeah, I guess you're right :)", -- [6]
							"/rchars", -- [7]
							"/gdisband", -- [8]
							"/1 Original meaning of gay is happy, so yeah, I guess you're right :)", -- [9]
							"/rchars", -- [10]
							"/gdisband", -- [11]
							"/rchars", -- [12]
							"/1 Original meaning of gay is happy, so yeah, I guess you're right :)", -- [13]
							"/gdisband", -- [14]
							"/1 Original meaning of gay is happy, so yeah, I guess you're right :)", -- [15]
							"/gdisband", -- [16]
							"/rchars", -- [17]
							"/1 Original meaning of gay is happy, so yeah, I guess you're right :)", -- [18]
							"/rchars", -- [19]
							"/gdisband", -- [20]
							"/1 Original meaning of gay is happy, so yeah, I guess you're right :)", -- [21]
							"/rchars", -- [22]
							"/gdisband", -- [23]
							"/rchars", -- [24]
							"/1 Original meaning of gay is happy, so yeah, I guess you're right :)", -- [25]
							"/gdisband", -- [26]
							"/1 Original meaning of gay is happy, so yeah, I guess you're right :)", -- [27]
							"/gdisband", -- [28]
							"/rchars", -- [29]
							"/1 Original meaning of gay is happy, so yeah, I guess you're right :)", -- [30]
							"/gdisband", -- [31]
							"/1 Original meaning of gay is happy, so yeah, I guess you're right :)", -- [32]
						},
						["Horde - Proudmoore"] = {
							"/focus", -- [1]
							" hahaha", -- [2]
							"/focus", -- [3]
							"/2 contact CS", -- [4]
							" hahaha", -- [5]
							"/focus", -- [6]
							" hahaha", -- [7]
							"/focus", -- [8]
							"/2 contact CS", -- [9]
							" hahaha", -- [10]
							"/focus", -- [11]
							" hahaha", -- [12]
							"/focus", -- [13]
							" hahaha", -- [14]
							"/focus", -- [15]
							" hahaha", -- [16]
							"/focus", -- [17]
							" hahaha", -- [18]
							"/focus", -- [19]
							"/2 contact CS", -- [20]
							" hahaha", -- [21]
							"/focus", -- [22]
							" hahaha", -- [23]
							"/focus", -- [24]
							" hahaha", -- [25]
							"/focus", -- [26]
							" hahaha", -- [27]
							"/focus", -- [28]
							" hahaha", -- [29]
							"/focus", -- [30]
							" hahaha", -- [31]
							"/focus", -- [32]
						},
						["Horde - Tichondrius"] = {
							"/reload", -- [1]
							"/p yeah I'm in afucked state agaion", -- [2]
							"/i thanks :)", -- [3]
							"/p then couldnt hear you", -- [4]
							"/reload", -- [5]
							"/i thanks :)", -- [6]
							"/reload", -- [7]
							"/p yeah I'm in afucked state agaion", -- [8]
							"/reload", -- [9]
							"/p yeah I'm in afucked state agaion", -- [10]
							"/i thanks :)", -- [11]
							"/reload", -- [12]
							"/p then couldnt hear you", -- [13]
							"/p yeah I'm in afucked state agaion", -- [14]
							"/reload", -- [15]
							"/p yeah I'm in afucked state agaion", -- [16]
							"/i thanks :)", -- [17]
							"/p then couldnt hear you", -- [18]
							"/reload", -- [19]
							"/i thanks :)", -- [20]
							"/reload", -- [21]
							"/p yeah I'm in afucked state agaion", -- [22]
							"/i thanks :)", -- [23]
							"/p then couldnt hear you", -- [24]
							"/reload", -- [25]
							"/i thanks :)", -- [26]
							"/reload", -- [27]
							"/p yeah I'm in afucked state agaion", -- [28]
							"/p then couldnt hear you", -- [29]
							"/i thanks :)", -- [30]
							"/p yeah I'm in afucked state agaion", -- [31]
							"/i thanks :)", -- [32]
						},
						["Alliance - Icecrown"] = {
							"/p ahve a good one!", -- [1]
							"/raid On Transistion", -- [2]
							"/cw Inevitabits-Icecrown o/", -- [3]
							" Aww, ok", -- [4]
							"/p summon available?", -- [5]
							"/p nice :)", -- [6]
							"/raid Sorry, my new phone arrived today, so that's my break :P", -- [7]
							"/p r", -- [8]
							"/raid Remember to join the community voice chat channel :)", -- [9]
							"/p I am heals", -- [10]
							" Peter is the master of all things", -- [11]
							"/p great group", -- [12]
							" baah", -- [13]
							"/p I have flames", -- [14]
							" great, lol", -- [15]
							"/p great group :)", -- [16]
							"/p thanks tho :)", -- [17]
							"/raid On Transistion", -- [18]
							" Aww, ok", -- [19]
							"/p nice :)", -- [20]
							"/p r", -- [21]
							"/p I am heals", -- [22]
							"/p great group", -- [23]
							"/p I have flames", -- [24]
							"/p great group :)", -- [25]
							"/raid On Transistion", -- [26]
							"/p nice :)", -- [27]
							"/p I am heals", -- [28]
							"/p I have flames", -- [29]
							"/raid On Transistion", -- [30]
							"/p I am heals", -- [31]
							"/raid On Transistion", -- [32]
						},
						["Alliance - Tichondrius"] = {
							"/1 Yeah in Zuldazar, but not here?", -- [1]
							"/cw Quadraxes-Darkspear same :P", -- [2]
							"/cw Kalithumos Still running keys", -- [3]
							"/cw Quadraxes-Darkspear same :P", -- [4]
							"/cw Quadraxes-Darkspear :P", -- [5]
							"/raid hence why I said ress me", -- [6]
							"/raid good luck", -- [7]
							"/cw Kalithumos Still running keys", -- [8]
							"/raid good luck", -- [9]
							"/2 No they are two different professions.", -- [10]
							"/1 Yeah in Zuldazar, but not here?", -- [11]
							"/cw Quadraxes-Darkspear Which is it?", -- [12]
							"/cw Quadraxes-Darkspear same :P", -- [13]
							" I ran another one", -- [14]
							"/cw Kalithumos Still running keys", -- [15]
							"/1 Yeah in Zuldazar, but not here?", -- [16]
							"/cw Quadraxes-Darkspear same :P", -- [17]
							"/cw Kalithumos Still running keys", -- [18]
							"/cw Quadraxes-Darkspear same :P", -- [19]
							"/cw Quadraxes-Darkspear :P", -- [20]
							"/raid good luck", -- [21]
							"/2 No they are two different professions.", -- [22]
							"/cw Quadraxes-Darkspear Which is it?", -- [23]
							" I ran another one", -- [24]
							"/1 Yeah in Zuldazar, but not here?", -- [25]
							"/cw Kalithumos Still running keys", -- [26]
							"/cw Quadraxes-Darkspear :P", -- [27]
							"/2 No they are two different professions.", -- [28]
							" I ran another one", -- [29]
							"/cw Kalithumos Still running keys", -- [30]
							"/2 No they are two different professions.", -- [31]
							"/cw Kalithumos Still running keys", -- [32]
						},
						["Horde - Bleeding Hollow"] = {
							" Also Prot Warrior deals a good amount of damage on AoE", -- [1]
							" Did Ragefire with a bunch of level 19-20s", -- [2]
							" Also Prot Warrior deals a good amount of damage on AoE", -- [3]
							" Did Ragefire with a bunch of level 19-20s", -- [4]
							"/i ok", -- [5]
							"/1 Heirlooms, got nerfed, not the other way around. :)", -- [6]
							" Did Ragefire with a bunch of level 19-20s", -- [7]
							" Also Prot Warrior deals a good amount of damage on AoE", -- [8]
							" Did Ragefire with a bunch of level 19-20s", -- [9]
							"/i ok", -- [10]
							" Did Ragefire with a bunch of level 19-20s", -- [11]
							"/i ok", -- [12]
							"/1 Heirlooms, got nerfed, not the other way around. :)", -- [13]
							" Did Ragefire with a bunch of level 19-20s", -- [14]
							" Also Prot Warrior deals a good amount of damage on AoE", -- [15]
							" Did Ragefire with a bunch of level 19-20s", -- [16]
							"/i ok", -- [17]
							"/1 Heirlooms, got nerfed, not the other way around. :)", -- [18]
							" Did Ragefire with a bunch of level 19-20s", -- [19]
							" Also Prot Warrior deals a good amount of damage on AoE", -- [20]
							" Did Ragefire with a bunch of level 19-20s", -- [21]
							"/i ok", -- [22]
							" Did Ragefire with a bunch of level 19-20s", -- [23]
							"/i ok", -- [24]
							"/1 Heirlooms, got nerfed, not the other way around. :)", -- [25]
							" Also Prot Warrior deals a good amount of damage on AoE", -- [26]
							"/i ok", -- [27]
							" Did Ragefire with a bunch of level 19-20s", -- [28]
							"/i ok", -- [29]
							"/1 Heirlooms, got nerfed, not the other way around. :)", -- [30]
							"/i ok", -- [31]
							"/1 Heirlooms, got nerfed, not the other way around. :)", -- [32]
						},
						["Horde - Hyjal"] = {
							" Hey :)", -- [1]
							"/bt4", -- [2]
							" Hey :)", -- [3]
							"/bt4", -- [4]
							" Hey :)", -- [5]
							"/bt4", -- [6]
							" Hey :)", -- [7]
							"/bt4", -- [8]
							" Hey :)", -- [9]
							"/bt4", -- [10]
							" Hey :)", -- [11]
							"/bt4", -- [12]
							" Hey :)", -- [13]
							"/bt4", -- [14]
							" Hey :)", -- [15]
							"/bt4", -- [16]
							" Hey :)", -- [17]
							"/bt4", -- [18]
							" Hey :)", -- [19]
							"/bt4", -- [20]
							" Hey :)", -- [21]
							"/bt4", -- [22]
							" Hey :)", -- [23]
							"/bt4", -- [24]
							" Hey :)", -- [25]
							"/bt4", -- [26]
							" Hey :)", -- [27]
							"/bt4", -- [28]
							" Hey :)", -- [29]
							"/bt4", -- [30]
							" Hey :)", -- [31]
							"/bt4", -- [32]
						},
					},
					["profiles"] = {
						["Base"] = {
							["Anchor"] = "BOTTOM",
							["Font"] = {
								["Font"] = "Prototype",
							},
						},
					},
				},
				["Buttons"] = {
					["profiles"] = {
						["Base"] = {
							["HideButtons"] = false,
						},
					},
				},
			},
			["profiles"] = {
				["Korugen - Icecrown"] = {
					["modules"] = {
					},
				},
				["Farahn - Icecrown"] = {
					["modules"] = {
					},
				},
				["Greybone - Icecrown"] = {
					["modules"] = {
					},
				},
				["Base"] = {
					["y"] = 43.6975598532504,
					["x"] = 33.3724626035666,
					["General"] = {
						["Font"] = {
							["Font"] = "Prototype",
						},
						["MinimalistTabs"] = false,
					},
					["modules"] = {
					},
				},
				["Kaldorath - Icecrown"] = {
					["modules"] = {
					},
				},
			},
		},
		["Bars"] = {
			["profiles"] = {
				["Korugen - Icecrown"] = {
					["StatesLoaded"] = true,
					["Bottombar5"] = {
						["State"] = {
							"5", -- [1]
							["Alt"] = "5",
							["Ctrl"] = "5",
						},
					},
					["Bottombar1"] = {
						["State"] = {
							"1", -- [1]
							["Alt"] = "1",
							["Ctrl"] = "1",
						},
					},
					["SidebarRight2"] = {
						["State"] = {
							"8", -- [1]
						},
					},
					["SidebarLeft1"] = {
						["State"] = {
							"9", -- [1]
						},
					},
					["Bottombar3"] = {
						["State"] = {
							"3", -- [1]
							["Alt"] = "3",
							["Ctrl"] = "3",
						},
					},
					["Bottombar2"] = {
						["State"] = {
							"2", -- [1]
							["Alt"] = "2",
							["Ctrl"] = "2",
						},
					},
					["Bottombar6"] = {
						["State"] = {
							"6", -- [1]
							["Alt"] = "6",
							["Ctrl"] = "6",
						},
					},
					["Bottombar4"] = {
						["State"] = {
							"4", -- [1]
							["Alt"] = "4",
							["Ctrl"] = "4",
						},
					},
					["SidebarLeft2"] = {
						["State"] = {
							"7", -- [1]
						},
					},
					["SidebarRight1"] = {
						["State"] = {
							"10", -- [1]
						},
					},
				},
				["Farahn - Icecrown"] = {
					["StatesLoaded"] = true,
					["Bottombar5"] = {
						["State"] = {
							"5", -- [1]
							["Alt"] = "5",
							["Ctrl"] = "5",
						},
					},
					["Bottombar1"] = {
						["State"] = {
							"1", -- [1]
							["Alt"] = "1",
							["Ctrl"] = "1",
						},
					},
					["SidebarRight2"] = {
						["State"] = {
							"8", -- [1]
						},
					},
					["SidebarLeft1"] = {
						["State"] = {
							"9", -- [1]
						},
					},
					["Bottombar3"] = {
						["State"] = {
							"3", -- [1]
							["Alt"] = "3",
							["Ctrl"] = "3",
						},
					},
					["Bottombar2"] = {
						["State"] = {
							"2", -- [1]
							["Alt"] = "2",
							["Ctrl"] = "2",
						},
					},
					["Bottombar6"] = {
						["State"] = {
							"6", -- [1]
							["Alt"] = "6",
							["Ctrl"] = "6",
						},
					},
					["Bottombar4"] = {
						["State"] = {
							"4", -- [1]
							["Alt"] = "4",
							["Ctrl"] = "4",
						},
					},
					["SidebarLeft2"] = {
						["State"] = {
							"7", -- [1]
						},
					},
					["SidebarRight1"] = {
						["State"] = {
							"10", -- [1]
						},
					},
				},
				["Kaldorath - Icecrown"] = {
					["StatesLoaded"] = true,
					["Bottombar5"] = {
						["State"] = {
							"5", -- [1]
							["Alt"] = "5",
							["Ctrl"] = "5",
						},
					},
					["Bottombar1"] = {
						["State"] = {
							"1", -- [1]
							["Alt"] = "1",
							["Ctrl"] = "1",
						},
					},
					["SidebarRight2"] = {
						["State"] = {
							"8", -- [1]
						},
					},
					["SidebarLeft1"] = {
						["State"] = {
							"9", -- [1]
						},
					},
					["Bottombar3"] = {
						["State"] = {
							"3", -- [1]
							["Alt"] = "3",
							["Ctrl"] = "3",
						},
					},
					["Bottombar2"] = {
						["State"] = {
							"2", -- [1]
							["Alt"] = "2",
							["Ctrl"] = "2",
						},
					},
					["Bottombar6"] = {
						["State"] = {
							"6", -- [1]
							["Alt"] = "6",
							["Ctrl"] = "6",
						},
					},
					["Bottombar4"] = {
						["State"] = {
							"4", -- [1]
							["Alt"] = "4",
							["Ctrl"] = "4",
						},
					},
					["SidebarLeft2"] = {
						["State"] = {
							"7", -- [1]
						},
					},
					["SidebarRight1"] = {
						["State"] = {
							"10", -- [1]
						},
					},
				},
				["Base"] = {
					["General"] = {
						["MacroFont"] = "Prototype",
						["ShowMacro"] = true,
						["ShowHotkey"] = true,
						["ShowEquipped"] = true,
						["CountFont"] = "Prototype",
						["HotkeyFont"] = "Prototype",
					},
					["BottomTexture"] = {
						["Y"] = 180,
						["Enable"] = false,
					},
					["SidebarRight2"] = {
						["State"] = {
							"2", -- [1]
						},
					},
					["StatesLoaded"] = true,
					["Bottombar5"] = {
						["State"] = {
							"8", -- [1]
							["Alt"] = "8",
							["Ctrl"] = "8",
						},
					},
					["VehicleExit"] = {
						["Y"] = -400,
						["X"] = -260,
					},
					["Bottombar4"] = {
						["State"] = {
							"7", -- [1]
							["Alt"] = "7",
							["Ctrl"] = "7",
						},
					},
					["ExtraActionBar"] = {
						["Y"] = 40,
						["X"] = 290,
					},
					["Bottombar3"] = {
						["State"] = {
							"1", -- [1]
							["Alt"] = "5",
							["Ctrl"] = "5",
						},
					},
					["Bottombar1"] = {
						["State"] = {
							"1", -- [1]
							"7", -- [2]
							"1", -- [3]
							[5] = "1",
							["Alt"] = "1",
							["Ctrl"] = "1",
						},
						["X"] = -450,
						["Fader"] = {
							["PowerClip"] = 0,
						},
						["Y"] = 32,
						["NumPerRow"] = 4,
					},
					["Bottombar2"] = {
						["State"] = {
							"2", -- [1]
							["Alt"] = "2",
							["Ctrl"] = "2",
						},
						["X"] = -450,
						["Y"] = 130,
						["NumPerRow"] = 4,
					},
					["Bottombar6"] = {
						["State"] = {
							"9", -- [1]
							["Alt"] = "9",
							["Ctrl"] = "9",
						},
					},
					["SidebarLeft1"] = {
						["State"] = {
							"3", -- [1]
						},
						["Anchor"] = "BT4Bar10",
					},
					["SidebarLeft2"] = {
						["State"] = {
							"1", -- [1]
						},
						["Anchor"] = "BT4Bar8",
					},
					["TopTexture"] = {
						["Y"] = 430,
						["X"] = 590,
						["Enable"] = false,
						["Alpha"] = 1,
					},
					["SidebarRight1"] = {
						["Enable"] = false,
						["Anchor"] = "BT4Bar9",
						["State"] = {
							"4", -- [1]
						},
					},
				},
				["Greybone - Icecrown"] = {
					["StatesLoaded"] = true,
					["Bottombar5"] = {
						["State"] = {
							"5", -- [1]
							["Alt"] = "5",
							["Ctrl"] = "5",
						},
					},
					["Bottombar1"] = {
						["State"] = {
							"1", -- [1]
							["Alt"] = "1",
							["Ctrl"] = "1",
						},
					},
					["SidebarRight2"] = {
						["State"] = {
							"8", -- [1]
						},
					},
					["SidebarLeft1"] = {
						["State"] = {
							"9", -- [1]
						},
					},
					["Bottombar3"] = {
						["State"] = {
							"3", -- [1]
							["Alt"] = "3",
							["Ctrl"] = "3",
						},
					},
					["Bottombar2"] = {
						["State"] = {
							"2", -- [1]
							["Alt"] = "2",
							["Ctrl"] = "2",
						},
					},
					["Bottombar6"] = {
						["State"] = {
							"6", -- [1]
							["Alt"] = "6",
							["Ctrl"] = "6",
						},
					},
					["Bottombar4"] = {
						["State"] = {
							"4", -- [1]
							["Alt"] = "4",
							["Ctrl"] = "4",
						},
					},
					["SidebarLeft2"] = {
						["State"] = {
							"7", -- [1]
						},
					},
					["SidebarRight1"] = {
						["State"] = {
							"10", -- [1]
						},
					},
				},
			},
		},
		["Cooldown"] = {
			["profiles"] = {
				["Base"] = {
					["Text"] = {
						["Font"] = "Prototype",
					},
					["General"] = {
						["MinDuration"] = 2,
					},
				},
			},
		},
		["InterruptAnnouncer"] = {
			["profiles"] = {
				["Base"] = {
					["General"] = {
						["EnableRaid"] = false,
						["EnablePet"] = false,
						["EnableParty"] = false,
					},
				},
			},
		},
	},
	["global"] = {
		["luiconfig"] = {
			["Praerend - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["vengeance"] = 2,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["Auras"] = 2,
					["lui"] = 3403,
					["mirrorbar"] = 1,
					["threatbar"] = 2,
					["bars"] = 2.4,
					["omen"] = 3300,
					["Micromenu"] = 1.5,
				},
			},
			["Rahran - Icecrown"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Fimwack - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["Auras"] = 2,
					["bars"] = 2.4,
					["threatbar"] = 2,
					["mirrorbar"] = 1,
				},
			},
			["Greybone - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["Auras"] = 2,
					["bars"] = 2.4,
					["threatbar"] = 2,
					["mirrorbar"] = 1,
				},
			},
			["Mallusof - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["vengeance"] = 2,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["Auras"] = 2,
					["lui"] = 3403,
					["mirrorbar"] = 1,
					["threatbar"] = 2,
					["bars"] = 2.4,
					["omen"] = 3300,
					["Micromenu"] = 1.5,
				},
			},
			["Rahran - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["Auras"] = 2,
					["bars"] = 2.4,
					["threatbar"] = 2,
					["mirrorbar"] = 1,
				},
			},
			["Farahn - Icecrown"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Praerend - Icecrown"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Tyindel - Frostwolf"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Mallusof - Tichondrius"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["vengeance"] = 2,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["Auras"] = 2,
					["lui"] = 3403,
					["mirrorbar"] = 1,
					["threatbar"] = 2,
					["bars"] = 2.4,
					["omen"] = 3300,
					["Micromenu"] = 1.5,
				},
			},
			["Deminath - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Kindhoof - Tichondrius"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Zethan - Tichondrius"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Alura - Tichondrius"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Korugen - Tichondrius"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Zaliga - Tichondrius"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Kaldorath - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["mirrorbar"] = 1,
					["bars"] = 2.4,
					["threatbar"] = 2,
					["Auras"] = 2,
				},
			},
			["Gorog - Bleeding Hollow"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Brity - Steamwheedle Cartel"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["threatbar"] = 2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["Auras"] = 2,
					["mirrorbar"] = 1,
				},
			},
			["Mallubank - Tichondrius"] = {
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["Micromenu"] = 1.5,
					["mirrorbar"] = 1,
					["bars"] = 2.4,
					["threatbar"] = 2,
					["Auras"] = 2,
				},
			},
			["Lorvol - Tichondrius"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Faerand - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["mirrorbar"] = 1,
					["bars"] = 2.4,
					["threatbar"] = 2,
					["Auras"] = 2,
				},
			},
			["Faerand - Icecrown"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Caircaidin - Frostwolf"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Rentror - Bleeding Hollow"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Fandagny - Frostwolf"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
					["Auras"] = 2,
				},
			},
			["Deminath - Hyjal"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["threatbar"] = 2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["Auras"] = 2,
					["mirrorbar"] = 1,
				},
			},
			["Morifa - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["mirrorbar"] = 1,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["threatbar"] = 2,
					["bars"] = 2.4,
					["omen"] = 3300,
					["Auras"] = 2,
				},
			},
			["Luugo - Frostwolf"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
					["Auras"] = 2,
				},
			},
			["Zearthan - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Korugen - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Evisca - Bleeding Hollow"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Gottagetlia - Aggramar"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["Auras"] = 2,
					["bars"] = 2.4,
					["threatbar"] = 2,
					["mirrorbar"] = 1,
				},
			},
			["Greybone - Icecrown"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Farahn - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["vengeance"] = 2,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["threatbar"] = 2,
					["bars"] = 2.4,
					["omen"] = 3300,
					["mirrorbar"] = 1,
				},
			},
			["Amranar - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["vengeance"] = 2,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["Micromenu"] = 1.5,
					["lui"] = 3403,
					["mirrorbar"] = 1,
					["threatbar"] = 2,
					["bars"] = 2.4,
					["omen"] = 3300,
					["Auras"] = 2,
				},
			},
			["Kaldorath - Icecrown"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Thedodrouk - Icecrown"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
					["Auras"] = 2,
				},
			},
			["Granjior - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["Auras"] = 2,
					["bars"] = 2.4,
					["threatbar"] = 2,
					["mirrorbar"] = 1,
				},
			},
			["Zaliga - Bleeding Hollow"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
					["threatbar"] = 2,
				},
			},
			["Miriyal - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["panels"] = 1.2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["Auras"] = 2,
					["bars"] = 2.4,
					["threatbar"] = 2,
					["mirrorbar"] = 1,
				},
			},
			["Kindhoof - Proudmoore"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Morifa - Tichondrius"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Korugen - Icecrown"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["threatbar"] = 2,
					["Auras"] = 2,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
				},
			},
			["Mallusof - Icecrown"] = {
				["IsConfigured"] = true,
				["Versions"] = {
					["raidmenu"] = 2.4,
					["swing"] = 2,
					["RaidMenu"] = true,
					["interrupt"] = 2,
					["threatbar"] = 2,
					["bartender"] = 3300,
					["lui"] = 3403,
					["Micromenu"] = 1.5,
					["bars"] = 2.4,
					["mirrorbar"] = 1,
					["Auras"] = 2,
				},
			},
		},
	},
	["profileKeys"] = {
		["Praerend - Proudmoore"] = "Base",
		["Rahran - Icecrown"] = "Base",
		["Fimwack - Proudmoore"] = "Base",
		["Greybone - Proudmoore"] = "Base",
		["Faerand - Proudmoore"] = "Base",
		["Rahran - Proudmoore"] = "Base",
		["Farahn - Icecrown"] = "Base",
		["Praerend - Icecrown"] = "Base",
		["Tyindel - Frostwolf"] = "Base",
		["Mallusof - Tichondrius"] = "Base",
		["Deminath - Proudmoore"] = "Base",
		["Rentror - Bleeding Hollow"] = "Base",
		["Zethan - Tichondrius"] = "Base",
		["Alura - Tichondrius"] = "Base",
		["Korugen - Tichondrius"] = "Base",
		["Zaliga - Tichondrius"] = "Base",
		["Kaldorath - Proudmoore"] = "Base",
		["Gorog - Bleeding Hollow"] = "Base",
		["Amranar - Proudmoore"] = "Base",
		["Lorvol - Tichondrius"] = "Base",
		["Korugen - Icecrown"] = "Base",
		["Faerand - Icecrown"] = "Base",
		["Fandagny - Frostwolf"] = "Base",
		["Deminath - Hyjal"] = "Base",
		["Kindhoof - Tichondrius"] = "Base",
		["Luugo - Frostwolf"] = "Base",
		["Zearthan - Proudmoore"] = "Base",
		["Morifa - Proudmoore"] = "Base",
		["Evisca - Bleeding Hollow"] = "Base",
		["Greybone - Icecrown"] = "Base",
		["Granjior - Proudmoore"] = "Base",
		["Farahn - Proudmoore"] = "Base",
		["Gottagetlia - Aggramar"] = "Base",
		["Kaldorath - Icecrown"] = "Base",
		["Thedodrouk - Icecrown"] = "Base",
		["Korugen - Proudmoore"] = "Base",
		["Zaliga - Bleeding Hollow"] = "Base",
		["Miriyal - Proudmoore"] = "Base",
		["Kindhoof - Proudmoore"] = "Base",
		["Morifa - Tichondrius"] = "Base",
		["Caircaidin - Frostwolf"] = "Base",
		["Mallusof - Icecrown"] = "Base",
	},
	["profiles"] = {
		["Korugen - Icecrown"] = {
			["modules"] = {
				["Merchant"] = false,
			},
		},
		["Default"] = {
		},
		["Farahn - Icecrown"] = {
			["modules"] = {
				["Merchant"] = false,
			},
		},
		["Greybone - Icecrown"] = {
			["modules"] = {
				["Merchant"] = false,
			},
		},
		["Base"] = {
			["modules"] = {
				["Merchant"] = true,
				["Cooldown"] = false,
				["Auras"] = true,
			},
			["General"] = {
				["HideTalentSpam"] = true,
				["DamageFont"] = "Prototype",
			},
			["Tooltip"] = {
				["ShowSex"] = true,
				["X"] = -2000,
				["Health"] = {
					["Texture"] = "LUI_Gradient",
				},
				["Y"] = -100,
			},
			["Minimap"] = {
				["Font"] = {
					["Font"] = "Prototype",
				},
				["Enable"] = false,
			},
		},
		["Kaldorath - Icecrown"] = {
			["modules"] = {
				["Merchant"] = false,
			},
		},
	},
}
LUIGold = nil
LUI_Themes = nil
LUI_Layouts = nil
