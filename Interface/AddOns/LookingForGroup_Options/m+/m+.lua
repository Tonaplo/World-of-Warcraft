...
LookingForGroup_Options.RegisterSearchPattern("find",function(profile,a,category,patterns)
	if category == 2 then
		local mpmin,mpmax = profile.minimum_mplus_level,profile.maximum_mplus_level
		if mpmin or mpmax then
			patterns[#patterns+1] = {matches = {LookingForGroup_Options.mythic_keystone}}
			if mpmin and mpmax and mpmin <= mpmax then
				local math_floor = math.floor
				while (mpmin~=0) do
					if mpmax < mpmin + 4 then
						local m = {}
						for i=mpmin,mpmax do
							m[#m+1] = i
						end
						patterns[#patterns+1] = {matches = m}
						break
					end
					mpmin=math_floor(mpmin/10)
					mpmax=math_floor(mpmax/10)
				end
			end
		end
	end
