...
--[[	if C_LFGList.CanCreateQuestGroup(quest_id) then
		...
		local kw = tostring(quest_id)
		coroutine.wrap(function()
			LookingForGroup_Options.Search("lfg_opt_sr_default_multiselect",
			nil,
			function() LookingForGroup_Options.find_quest_group(quest_id) end,
			{"spam"},
			categoryID,{{matches={quest_id}}},C_PvP.IsWarModeActive() and LE_LFG_LIST_FILTER_PVP or LE_LFG_LIST_FILTER_PVE,0,nil,function()
				if LFGListFrame.EntryCreation.Name:GetText():match("%d+")==kw then
