...
function CONDITIONERDROPDOWNMENU_Initialize(frame, initFunction, displayMode, level, menuList)
	...
	--securecall("CONDITIONERDROPDOWNMENU_InitializeHelper", frame);
...
local CONDITIONERDROPDOWNMENU_SecureInfo = {};
...
function CONDITIONERDROPDOWNMENU_CreateInfo()
	...
	--if ( issecure() ) then
	--	securecall(wipe, CONDITIONERDROPDOWNMENU_SecureInfo);
	--	return CONDITIONERDROPDOWNMENU_SecureInfo;
	--else
...
function CONDITIONERDROPDOWNMENU_AddButton(info, level)
	...
	if ( not CONDITIONERDROPDOWNMENU_OPEN_MENU ) then
		return;
	end
	]]
	if ( not level ) then
