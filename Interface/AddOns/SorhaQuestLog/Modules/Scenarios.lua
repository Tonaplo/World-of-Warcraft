...
function SQLScenario:Update()
	...
	self.IsChallengeMode = bit.band(flags, LE_SCENARIO_TYPE_CHALLENGE_MODE) == LE_SCENARIO_TYPE_CHALLENGE_MODE;
	self.IsProvingGrounds = bit.band(flags, LE_SCENARIO_TYPE_PROVING_GROUNDS) == LE_SCENARIO_TYPE_PROVING_GROUNDS;
	self.IsDungeon = bit.band(flags, LE_SCENARIO_TYPE_USE_DUNGEON_DISPLAY) == LE_SCENARIO_TYPE_USE_DUNGEON_DISPLAY;
...
function ScenarioTracker:UpdateMinion()
	...
	if (objScenario:IsActive() and objScenario.IsProvingGrounds == false) then
		...
		if (objScenario.CurrentStage > 0 and (objScenario.CurrentStage <= objScenario.Stages or objScenario.BonusCount > 0)) then		
			...
			if (objScenario.IsChallengeMode and objScenario.Timer.Running == false) then
