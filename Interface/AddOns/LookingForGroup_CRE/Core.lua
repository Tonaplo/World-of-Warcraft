...
local function search()
	...
	if profile.a.current_map then
		...
		local activityName, shortName, categoryID, groupID, itemLevel, ft = C_LFGList.GetActivityInfo(current_activity)
		kw = activityName or shortName
		filters = ft
		category=categoryID
	end
	coroutine.wrap(function()
		...
		search,
		{"spam","cr"},
		category,{{matches={kw}}},filters,0,converter,nil,nil,{"cr"})
	end)()
end
