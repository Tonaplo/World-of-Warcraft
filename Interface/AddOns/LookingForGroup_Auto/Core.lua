...
function LookingForGroup.accepted(name,search,create,secure,raid,quest_id,keyword)
	...
	if count~=0 and not iscache then
		...
		for i=1,#results do
			...
			elseif not isDelisted and autoaccept and LookingForGroup.auto_ilvl(itemLevel)<=iLvl and (raid or numMembers < 5) then
				local yes
				if keyword == nil then
					yes = true
				elseif name then
	...
	if hardware then
		if iscache then
			...
			if #results~=0 then
				...
				LookingForGroup:UnregisterEvent("LFG_LIST_APPLICATION_STATUS_UPDATED")
				if yd then
					LookingForGroup:RegisterEvent("LFG_LIST_JOINED_GROUP",function()
						coroutine.resume(current,true)
					end)
					LookingForGroup:RegisterEvent("LFG_LIST_APPLICATION_STATUS_UPDATED",function(event,applicationid,invited,applied)
					...
					local yd = coroutine.yield()
					LookingForGroup:UnregisterEvent("LFG_LIST_JOINED_GROUP")
					LookingForGroup:UnregisterEvent("LFG_LIST_APPLICATION_STATUS_UPDATED")
